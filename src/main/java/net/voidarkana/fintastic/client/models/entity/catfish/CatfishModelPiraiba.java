...
public class CatfishModelPiraiba<T extends Catfish> extends FintasticModel<T> {
	...
	public void setupAnim(T entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		...
		if (this.young)
			limbSwing /= 2;
		this.animateIdle(entity.idleAnimationState, CatfishAnims.POSE_PIRAIBA, ageInTicks, 1.0F, Math.max(0, 1-entity.getTicksOutsideWater()/3f));
		...
		this.animateIdle(entity.idleAnimationState, CatfishAnims.IDLE_PIRAIBA, ageInTicks, 1.0F, Math.max(0, 1-entity.getTicksOutsideWater()/3f-Math.abs(limbSwingAmount)));

		this.animateIdle(entity.idleAnimationState, CatfishAnims.FLOP, ageInTicks, 1.0f, entity.getTicksOutsideWater()/3f);
		this.animateIdle(entity.idleAnimationState, CatfishAnims.FLOP_PIRAIBA, ageInTicks, 1.0f, entity.getTicksOutsideWater()/3f);

		this.animateWalk(CatfishAnims.SWIM, limbSwing*3, limbSwingAmount*5f, 2f, 3f*(1-(entity.getTicksOutsideWater()/3f)));
		this.animateWalk(CatfishAnims.SWIM_PIRAIBA, limbSwing*3, limbSwingAmount*5f, 2f, 3f*(1-(entity.getTicksOutsideWater()/3f)));
