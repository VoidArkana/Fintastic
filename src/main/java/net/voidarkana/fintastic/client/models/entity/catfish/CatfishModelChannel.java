...
public class CatfishModelChannel<T extends Catfish> extends FintasticModel<T> {
	...
	public void setupAnim(T pEntity, float pLimbSwing, float pLimbSwingAmount, float pAgeInTicks, float netHeadYaw, float headPitch) {
		...
		this.animateIdle(pEntity.idleAnimationState, CatfishAnims.FLOP_CHANNEL, pAgeInTicks, 1.0f, pEntity.getTicksOutsideWater()/3f);

		if (pEntity.isInWaterOrBubble()){
			this.animateWalk(CatfishAnims.SWIM, pLimbSwing*3, pLimbSwingAmount*5f, 2f, 3f);
		}
