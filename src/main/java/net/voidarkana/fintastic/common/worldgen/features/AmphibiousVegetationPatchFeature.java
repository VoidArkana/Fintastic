...
public class AmphibiousVegetationPatchFeature extends Feature<VegetationPatchConfiguration> {
    ...
    public AmphibiousVegetationPatchFeature(Codec<VegetationPatchConfiguration> codec) {
        super(codec);
    }
        ...
        for(int i = -xRadius; i <= xRadius; ++i) {
            ...
            for(int j = -zRadius; j <= zRadius; ++j) {
                ...
                if (!flag3 && (!flag4 || config.extraEdgeColumnChance != 0.0F && !(random.nextFloat() > config.extraEdgeColumnChance))) {
                    ...
                    for(int k = 0; level.isStateAtPosition(blockpos$mutableblockpos, (blockState) -> blockState.is(Blocks.WATER) || blockState.isAir()) && k < config.verticalRange; ++k) {

                        blockpos$mutableblockpos.move(direction);
                    }

                    for(int i1 = 0; level.isStateAtPosition(blockpos$mutableblockpos, (blockState) -> !blockState.is(Blocks.WATER) || blockState.isAir()) && i1 < config.verticalRange; ++i1) {
                    ...
                    if ((level.isEmptyBlock(blockpos$mutableblockpos) || level.getBlockState(blockpos$mutableblockpos).is(Blocks.WATER) || level.getBlockState(blockpos$mutableblockpos).isAir())
