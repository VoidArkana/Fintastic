...
public class UnderwaterVegetationPatchFeature extends Feature<VegetationPatchConfiguration> {
    ...
    public UnderwaterVegetationPatchFeature(Codec<VegetationPatchConfiguration> pCodec) {
        super(pCodec);
    }
        ...
        for(int i = -pXRadius; i <= pXRadius; ++i) {
            ...
            for(int j = -pZRadius; j <= pZRadius; ++j) {
                ...
                if (!flag3 && (!flag4 || pConfig.extraEdgeColumnChance != 0.0F && !(pRandom.nextFloat() > pConfig.extraEdgeColumnChance))) {
                    ...
                    if ((pLevel.isEmptyBlock(blockpos$mutableblockpos) || pLevel.getBlockState(blockpos$mutableblockpos).is(Blocks.WATER))
