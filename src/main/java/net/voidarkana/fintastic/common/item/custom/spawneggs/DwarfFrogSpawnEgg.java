...
public class DwarfFrogSpawnEgg extends FishVariantSpawnEggItem {

    public DwarfFrogSpawnEgg(Supplier<? extends EntityType<? extends Mob>> type, int backgroundColor, int highlightColor, Properties props) {
        super(type, backgroundColor, highlightColor, props);
    }

    @Override
    public void applyEntityVariant(ItemStack itemstack, Entity entity) {
        if (entity instanceof DwarfFrog fish && itemstack.hasTag())
            if (itemstack.getTag().contains(DATA_CREATURE))
                if (itemstack.getTag().getInt(DATA_CREATURE)!=-1){
                    ...
                    DwarfFrog.FrogVariant variant = DwarfFrog.FrogVariant.values()[i];
                    if (variant != null){
    ...
    public void changeEntityVariant(ItemStack stack){
        int length = DwarfFrog.FrogVariant.values().length-1;
        int currentIndex = -1;
        int newIndex;
        ...
        if (itemstack.hasTag()){
            if (itemstack.getTag().contains(DATA_CREATURE)){
                if (itemstack.getTag().getInt(DATA_CREATURE)!=-1){
                    ...
                    DwarfFrog.FrogVariant variant = DwarfFrog.FrogVariant.values()[i];
                    if (variant != null) {
                        String common = "fintastic.dwarf_frog_common." + variant.getSerializedName();
