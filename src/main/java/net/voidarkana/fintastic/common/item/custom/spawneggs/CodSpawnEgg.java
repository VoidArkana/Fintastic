...
public class CodSpawnEgg extends FishVariantSpawnEggItem {

    public CodSpawnEgg(Supplier<? extends EntityType<? extends Mob>> type, int backgroundColor, int highlightColor, Properties props) {
        super(type, backgroundColor, highlightColor, props);
    }

    @Override
    public void applyEntityVariant(ItemStack itemstack, Entity entity) {
        if (entity instanceof FintasticCod fish && itemstack.hasTag())
            if (itemstack.getTag().contains(DATA_CREATURE))
                if (itemstack.getTag().getInt(DATA_CREATURE)!=-1){
                    ...
                    FintasticCod.CodVariant variant = FintasticCod.CodVariant.values()[i];
                    if (variant != null){
    ...
    public void changeEntityVariant(ItemStack stack){
        int length = FintasticCod.CodVariant.values().length-1;
        int currentIndex = -1;
        int newIndex;
        ...
        if (itemstack.hasTag()){
            if (itemstack.getTag().contains(DATA_CREATURE)){
                if (itemstack.getTag().getInt(DATA_CREATURE)!=-1){
                    ...
                    FintasticCod.CodVariant variant = FintasticCod.CodVariant.values()[i];
                    if (variant != null) {
                        String common = "fintastic.cod_common." + variant.getSerializedName();
