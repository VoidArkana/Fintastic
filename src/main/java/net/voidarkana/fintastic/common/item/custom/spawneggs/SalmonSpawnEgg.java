...
public class SalmonSpawnEgg extends FishVariantSpawnEggItem {

    public SalmonSpawnEgg(Supplier<? extends EntityType<? extends Mob>> type, int backgroundColor, int highlightColor, Properties props) {
        super(type, backgroundColor, highlightColor, props);
    }

    @Override
    public void applyEntityVariant(ItemStack itemstack, Entity entity) {
        if (entity instanceof FintasticSalmon fish && itemstack.hasTag())
            if (itemstack.getTag().contains(DATA_CREATURE))
                if (itemstack.getTag().getInt(DATA_CREATURE)!=-1){
                    ...
                    FintasticSalmon.SalmonVariant variant = FintasticSalmon.SalmonVariant.values()[i];
                    if (variant != null){
                        fish.setVariant(variant.getVariant());
                    }
                }
    }

    public void changeEntityVariant(ItemStack stack){
        int length = FintasticSalmon.SalmonVariant.values().length-1;
        int currentIndex = -1;
        int newIndex;
        ...
        if (itemstack.hasTag()){
            if (itemstack.getTag().contains(DATA_CREATURE)){
                if (itemstack.getTag().getInt(DATA_CREATURE)!=-1){
                    ...
                    FintasticSalmon.SalmonVariant variant = FintasticSalmon.SalmonVariant.values()[i];
                    if (variant != null) {
                        String common = "fintastic.salmon_common." + variant.getSerializedName();
